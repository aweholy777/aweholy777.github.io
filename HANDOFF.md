...
- **上傳＋發布＝每小時排程滴傳**：任務名 `QT-Upload-5090`（觸發：**每小時 :05** ＋ **每次登入後 5 分鐘**，後者確保關機重開／登出再登入後自動接續進度），跑 `D:\qtproject\tasks\5090-migration\upload_5090.ps1`
...
### 現況快照（2026-09-24，供參考，會過期，請自行重新統計 `tasks/progress/_summary.txt` 取得最新數字）

- **全部影片已生成完畢（2026-09-24 03:27，最後一批 exit=0）**：新約 849 ＋ 舊約 1862 ＝ **2711 篇，未生成 0**。
  生成用的 `QT-GenLoop-5090` **已停用**（`QT-GenOnce-5090` 亦閒置）；除非日後新增 QT 文章，否則不需再生成。
- 新約 849 篇：已上傳 849 → **新約全數完成（生成＋上傳）**。
- 舊約 1862 篇：已上傳 292、已生成未上傳 1570、未生成 0 → **只剩上傳**。
- **目前唯一的工作＝YouTube 上傳**：約 23–24 支/天（`DailyCap=24` 滴傳），舊約剩餘約 1570 支，
  約需 **2 個月出頭（估 ~12 月初）** 傳完。
