...
### 現況快照（2026-09-13，供參考，會過期，請自行重新統計 `tasks/progress/_summary.txt` 取得最新數字）

- 新約 849 篇：**已上傳 849、已生成未上傳 0、未生成 0 → 新約全數完成（生成＋上傳都結束）**。
- 舊約 1862 篇：已上傳 41（已從創世記開始按卷序上傳）、已生成未上傳 1511、未生成 310
  （未生成集中在以賽亞書尾段之後的後段先知書一帶）。
- 上傳速率約 23–24 支/天（`DailyCap=24` 滴傳），是目前最大瓶頸；舊約剩餘上傳約需 2.5 個月。
- 生成速率約 24 篇/批、每批約 17–21 小時（批次間休息 1 小時），約 30 篇/天。
