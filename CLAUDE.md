...
- **5090 執行節點**（192.168.68.57，DESKTOP-BFSJ95H）：只做影片生成 + YouTube 上傳。
  本機跑 ComfyUI（127.0.0.1:8188），`nightly_head.py --server local` 生成，
  `yt_publish.py --auto --no-push` 上傳並把 `{{< youtube ID >}}` 嵌進它生成的那篇 QT，
  寫入 `video-pipeline/yt_uploaded.csv`，然後只 push「那幾篇 + csv」到 main。
  它的機器專屬指令在 5090 的 `CLAUDE.local.md`（gitignored，不在此 repo）。
- **本機 3060（你，軍師）**：QT 內容、排程、`_index` 維護，**以及網站建置與發布**。

**你（3060）在影片這條線上的職責**：
1. `git pull`，取得 5090 推上來的 `yt_uploaded.csv` 與已嵌入 shortcode 的 QT 文章。
2. `hugo --buildFuture` 建置，確認無錯。
3. 推發布副本到 GitHub Pages（沿用既有發布流程；5090 上傳時都帶 `--no-push`，
   發布一律由你做）。
